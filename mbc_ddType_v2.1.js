		...
		function validate(){
		   ...
		   */
		   jQuery(".liSelection").each( function(){
			   ...
			   if (!(thisvalue!="" && thisvalue<=36)){ jQuery(this).addClass("error");error++; error_string += "<b><li><span style='font-weight:900 !important;'>Please review input range (0-36) for "+jQuery(this).parent().find(".liLabel").text()+".</span></b></li>"  } else { jQuery(this).removeClass("error"); } 
		   });
				...
				total_number:0
					...
					if ( originalglazed > 0 ){
						...
						if ( originalglazed < 3){
							...
							if (settings.debug) console.log("============original glazed===================");
					...
					if (assorted.length==0){
					    ...
					    if ( excess_size >  0 ){ //assorted with originzal glazed
								bIsAssorted = true;
								for(i=1;i<=excess_size;i++){
						...
						if ( assorted.length < 3 ){
							...
							var initsort = sortpacking(number_of_items, packing_types);
								...
								while ( assorted_excess >  lowestpack) {
								   var newsort = sortpacking(assorted_excess, packing_types)
