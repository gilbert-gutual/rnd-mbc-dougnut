   ...
   function onLoad() {
	    ...
	    $("ul.cm-numeric-response-set").find("li").each( function(){
		   ...
		   var label = data[0] 
			   ...
			   tbldata[label][group[0]] = group[1].replace("'","").replace("}","").replace(" ","_");
		...
		$.each(tbldata, function(item,data){
		  ...
		  var template = "<div class='menu-item'><div class='liSelection' data-price='"+rawprice[0]+"' data-code='"+data.itemNumber+"' data-value=''><select name='answer' class='round'><option value=''></option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option><option value='8'>8</option><option value='9'>9</option></select></div><div class='liLabel'>"+item+" "+tooltip+"</div><div class='liPrices'>"+(price1+price2)+"</div></div>";
